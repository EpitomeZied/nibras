...
- Student project tracking UI now lives at `apps/web/app/(app)/projects/`.
...
- `POST /v1/tracking/projects/:projectId/applications` (team role applications)
...
- The `/projects` page is the student-facing project hub: course portfolio overview, per-course workspace with milestones, team formation, rubric, and resources.
- Instructor/admin web screens live under `/instructor/courses/.../projects`.
- Submission types supported in the tracking UI are `github`, `link`, and `text`.
- Team projects support role applications, team generation, and locked rosters.
- File uploads are not supported in the tracking submission UI.
